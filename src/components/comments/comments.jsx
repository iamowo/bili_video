...
function Comments(props) {
  const vid = props.vid
  const did = props.did
  const commentType = vid !== undefined ? 0 : 1          // 0视频 1动态
  const userinfo = props.userinfo
  const uid = props.uid
  const hisuid = props.hisuid                            // 视频up 的uid
  ...
  useEffect(() => {
    const getData1 = async () => {      
      const res = await getAllComment(vid, uid, 0, commentType)
      setCommentlist(res)      
